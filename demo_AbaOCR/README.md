...
`i18n.csv` 中规定了每个字符串及对应的多语言翻译。以下示例，翻译了英文、繁中、日语：

```
key,     en_US,   zh_TW,  ja_JP
字符串1, String 1, 字串1, 文字列1
字符串2, String 2, 字串2, 文字列2
```

可以只翻译为英文。非中文的语言（如日韩俄德法……）均会采用英文翻译。
...
文件夹名称唯一标识一个插件。文件夹名须为Ascii字符。

文件夹中必须有一个 `__init__.py` 。Umi会读取并载入该py文件，来导入插件。
